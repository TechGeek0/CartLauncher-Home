...
export const svgpayment =
  ...
  export const getVisibleProducts = (
    products,
    { sortBy, category, storeLocation, brand, color, rating, value, condition }
    ) => {
    return products
      .filter((item) => {
        let catResult = false,
        ...
        if (category && category.length > 0) {
          for (let i = 0; i < category.length; i++) {
            if (item.category._id === category[i] || category[i] === "1")
              catResult = true;
          }
        } else {
          catResult = true;
